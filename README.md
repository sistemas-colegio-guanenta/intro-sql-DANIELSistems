...
`SELECT COUNT(*) AS Numero_Articulos FROM Articulo WHERE precio_articulo >= 50000;`
...
`SELECT nombre_articulo AS Articulo, precio_articulo FROM Articulo WHERE precio_articulo >= 50000 ORDER BY precio_articulo DESC, nombre_articulo ASC;`
...
`SELECT A.*, F.* FROM Articulo A JOIN Fabricante F ON A.id_fabricante = F.id_fabricante;`
...
`SELECT A.nombre_articulo AS Articulo, A.precio_articulo, F.nombre_fabricante FROM Articulo A JOIN Fabricante F ON A.id_fabricante = F.id_fabricante;`
...
`SELECT codigo_fabricante, AVG(precio_articulo) AS Precio_Promedio FROM Articulo GROUP BY codigo_fabricante;`
